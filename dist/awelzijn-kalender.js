...
;angular.module('awelzijn.kalender').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/kalender.html',
    ...
    "<span class=small ng-show=dag.eersteOfLaatsteVanMaand> {{dag.datum | date:'MMM'}}</span> </div> <div ng-if=\"dag.activiteiten.length > 0\" class=heeftActiviteiten></div> <div ng-if=dag.activiteiten class=toon-info> <div ng-click=ctrl.onClick(activiteit) ng-repeat=\"activiteit in dag.activiteiten\" class=\"kalenderActiviteit {{activiteit.id}}\"> <div ng-show=ctrl.toonUur> <input ng-click=$event.stopPropagation(); type=checkbox ng-model=activiteit.checked ng-trim=\"false\">\n" +
    "{{ctrl.getStringWithDots(activiteit.naam)}}[{htmlmin-lb}]<span class=dgpSpanKalender>{{activiteit.datumVan | date:'HH:mm'}}</span> </div> <div ng-show=!ctrl.toonUur> <input ng-click=$event.stopPropagation(); type=checkbox ng-model=\"activiteit.checked\">{{activiteit.naam}} </div> </div> </div> </td> </tr> </tbody> </table> <div class=activiteitenPerDag ng-show=ctrl.geselecteerdeDag> <div ng-click=ctrl.onClick(activiteit) class=\"activiteit hover clearfix\" ng-repeat=\"activiteit in ctrl.geselecteerdeDag.activiteiten\"> <div> <input ng-click=$event.stopPropagation(); type=checkbox ng-model=activiteit.checked ng-trim=\"false\">\n" +
